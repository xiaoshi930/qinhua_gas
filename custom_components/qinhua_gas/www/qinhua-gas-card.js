console.info("%c 消逝卡-燃气卡 \n%c        v 1.5 ", "color: red; font-weight: bold; background: black", "color: white; font-weight: bold; background: black");
...
window.customCards.push(
  ...
  {
    type: 'xiaoshi-qinhua-gas-button',
    name: '秦华燃气按钮',
    description: '显示燃气余额按钮，点击弹出燃气详情卡片',
    preview: true
  }
);
...
class XiaoshiQinhuaGasCard extends LitElement {
 ...
 /*渲染日度用气条形图*/
  ...
  renderMain() {
    ...
    const Shadow = theme === 'on' ? '0 1px 2px rgba(255, 255, 255, 0.3)' : '0 1px 2px rgba(50, 50, 50, 0.6)';
    const svgpath ='/qinhua_gas-local/icon/qinhua-gas.png';
...
class XiaoshiQinhuaGasButtonEditor extends LitElement {
  static get properties() {
    return {
      hass: { type: Object },
      config: { type: Object },
  ...
  render() {
    ...
    return html`
      <div class="form">
        ...
        <div class="form-group">
          <label>小数点精度：控制显示的小数位数，默认2位</label>
          <input type="number" @change=${this._entityChanged} .value=${this.config.decimal_precision !== undefined ? this.config.decimal_precision : '2'} name="decimal_precision" placeholder="默认2" min="0" max="10" step="1" />
        </div>

        <div class="checkbox-group">
          <input type="checkbox" @change=${this._entityChanged} .checked=${this.config.tablet_mode === true} name="tablet_mode" id="tablet_mode" />
          <label for="tablet_mode" class="checkbox-label">平板端特性（透明背景、白色文字、显示燃气费余额和预计天数）</label>
        </div>
        ...
        <div class="form-group">
          <label>弹窗宽度：默认auto</label>
          <input type="text" @change=${this._entityChanged} .value=${this.config.popup_width !== undefined ? this.config.popup_width : 'auto'} name="popup_width" placeholder="默认auto" />
        </div>

        <div class="form-group">
          <label>弹窗位置：默认50%居中</label>
          <input type="text" @change=${this._entityChanged} .value=${this.config.popup_top !== undefined ? this.config.popup_top : '50%'} name="popup_top" placeholder="默认50%" />
        </div>
  ...
  _entityChanged(e) {
    ...
    else if (name === 'decimal_precision') finalValue = value !== undefined ? parseInt(value) : 2;
...
customElements.define('xiaoshi-qinhua-gas-button-editor', XiaoshiQinhuaGasButtonEditor);

class XiaoshiQinhuaGasButton extends LitElement {
  static get properties() {
    return {
      hass: Object,
      config: Object,
  ...
  static getConfigElement() {
    return document.createElement("xiaoshi-qinhua-gas-button-editor");
  }
  ...
  _handleButtonClick() {
    ...
    this._showNativePopup({ type: 'custom:xiaoshi-qinhua-gas-info', ...stateGridCardConfig });
    this._handleClick();
  }
  ...
  static _injectPopupStyles() {
    if (XiaoshiQinhuaGasButton._stylesInjected) return;
    XiaoshiQinhuaGasButton._stylesInjected = true;
    const style = document.createElement('style');
    style.id = 'xiaoshi-qinhua-gas-button-popup-style';
    style.textContent = `@keyframes xiaoshiQinhuaGasButtonPopupIn { from { opacity: 0; scale: 0.95; } to { opacity: 1; scale: 1; } }`;
  ...
  _showNativePopup(popupContent) {
    ...
    if (!hassObj) { console.error('[XiaoshiQinhuaGasButton] 无法获取 hass 对象'); return; }
    ...
    popup.style.cssText = `position: fixed; top: ${popupTop}; left: 50%; transform: ${popupTransform}; z-index: 1005; background: transparent; padding: 0; width: ${popupWidth}; max-width: 100vw; max-height: 100vh; overflow: hidden; box-sizing: border-box; animation: xiaoshiQinhuaGasButtonPopupIn 0.2s ease-out;`;
  ...
  async _createPopupCard(container, cardConfig, hassObj) {
    ...
    } catch (err) { console.error('[XiaoshiQinhuaGasButton] 创建弹窗卡片失败:', err); container.innerHTML = `<div style="color:red;padding:20px;">加载失败: ${err.message}</div>`; }
  }
  ...
  _startPopupHassWatcher(hassObj) {
    ...
    } catch (err) { console.error('[XiaoshiQinhuaGasButton] 订阅状态变化失败:', err); }
  }
  ...
  _updatePopupCard() {
    if (this._popupCardElement && this._popupHass) {
      try { this._popupCardElement.hass = this._popupHass; } catch (err) { console.warn('[XiaoshiQinhuaGasButton] 弹窗卡片更新失败:', err.message); }
    }
  }
  ...
  _computeDisplayValue() {
    ...
    const decimalPrecision = this.config.decimal_precision !== undefined ? parseInt(this.config.decimal_precision) : 2;
  ...
  render() {
    ...
    if (tabletMode) {
      ...
      return html`
        <div class="balance-status tablet-mode" style="--fg-color: ${tabletFgColor}; --bg-color: ${buttonBgColor};" @click=${this._handleButtonClick}>
          <span class="tablet-balance"><span class="status-emoji">${buttonEmoji}</span><span style="color: ${tabletFgColor};">燃气余额：<span style="color: ${balanceColor};">${balanceValue}元</span></span></span>
    ...
    if (typeof displayValue === 'number') {
      const dp = this.config.decimal_precision !== undefined ? parseInt(this.config.decimal_precision) : 2;
...
customElements.define('xiaoshi-qinhua-gas-button', XiaoshiQinhuaGasButton);
