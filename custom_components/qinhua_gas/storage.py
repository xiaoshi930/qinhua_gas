"""Persistent storage for Qinghua Gas integration."""
import json
import logging
import os
...
class QinhuaGasStorage:
    """Manage persistent JSON storage for gas data.
    ...
    def __init__(self, hass, card_id: str):
        """Initialize storage."""
        self._hass = hass
        self._card_id = card_id
        self._file_path = hass.config.path(f"qinhua_gas_{card_id}.json")
    ...
    def _load_sync(self) -> None:
        ...
        try:
            if os.path.exists(self._file_path):
                ...
                _LOGGER.info(
                    "已加载燃气持久化数据: %s (dayList=%d条, monthList=%d条, yearList=%d条)",
                    self._file_path,
                ...
                _LOGGER.info("燃气持久化文件不存在，初始化空数据: %s", self._file_path)
        except (json.JSONDecodeError, IOError) as ex:
            _LOGGER.error("加载燃气持久化数据失败: %s", ex)
            self._data = {
                "date": "",
                "balance": 0,
                "dayList": [],
    ...
    def _save_sync(self) -> None:
        ...
        try:
            ...
            _LOGGER.debug("已保存燃气持久化数据: %s", self._file_path)
        except IOError as ex:
            _LOGGER.error("保存燃气持久化数据失败: %s", ex)
    ...
    def update(self, new_data: dict[str, Any]) -> dict[str, Any]:
        ...
        _LOGGER.info(
            "燃气数据已合并并持久化: dayList=%d条, monthList=%d条, yearList=%d条",
            len(self._data.get("dayList", [])),
            len(self._data.get("monthList", [])),
            len(self._data.get("yearList", [])),
        )
